...
class TestDb(unittest.TestCase):
    @requests_mock.mock()
    def test_find_names(self, r):
        """
            Test: Unit: RequestJarbas: find_names
        """
    ...
    @requests_mock.mock()
    def test_find_suspicions(self, r):
        """
            Test: Unit: RequestJarbas: find_suspicions
        """
        req = Requester()

        url = "%s?suspicions=1&applicant_id=%s&limit=7&offset=" % (os.environ.get("JARBAS_HOST"), 123)
        ...
        expected_suspicions = [
            ...
            },
            {
                ...
                "receipt_text": "null",
                "search_vector": "'aprendizag':6A 'colatt':2A 'comercial':7A 'congressperson':11 'meal':12"
                " 'nacional':4A 'pmdb':9A 'sc':10B 'senac':8A 'servic':3A 'vald':1A",
            },
            {
            ...
            },
            {
                ...
                "suspicions": {"meal_price_outlier": True},
                "receipt_text": "null",
                "search_vector": "'03346671000954':8A 'cd':2A 'congressperson':9 'epp':7A 'lid.gov':1A 'ltda':6A"
                " 'meal':10 'self':4A 'servic':5A 'taiob':3A",
            },
            {
                ...
                "suspicions": {"meal_price_outlier": True},
                "receipt_text": "null",
                "search_vector": "'18440959168':7A '27570249000199':8A 'antoni':4A 'borg':6A "
                "'congressperson':9 'girott':5A 'lideranc':1A 'meal':10 'pt':3A",
            },
        ]
        r.get(url, json=mock_json_jarbas_suspicions())

        suspicions, offset = req.find_suspicions(123)

        self.assertEqual(suspicions, expected_suspicions)
